...
balls.prototype.render = function(){
  g_ctx.fillStyle = this.b_color;
  fillCircle(g_ctx, this.b_x,this.b_y,this.b_radius);

}

balls.prototype.update = function(){ 

  this.b_x += this.b_velX;
  this.b_y += this.b_velY;

  var r =this.b_radius;

  if(this.b_x < 0 + r || this.b_x > g_canvas.width - r){
    this.b_velX *= -1;
  }
  if(this.b_y < 0 + r || this.b_y > g_canvas.height - r){
    this.b_velY *= -1;
  }
}
...
Ship.prototype.applyAccel = function(accelX, accelY, du) {
  ...
  if (g_useGravity) {
    ...
    var minY = g_shipSprite.height / 2;
...
Ship.prototype.wrapPosition = function() {
  ...
  this.cy = g_ship.wrapRange(this.cy, 0, g_canvas.height);
	this.cx = g_ship.wrapRange(this.cx, 0, g_canvas.width);
 
 
};

Ship.prototype.wrapRange = function(value, lowBound, highBound) {
...
Ship.prototype.render = function(ctx) {
  ...
  // YOUR STUFF HERE
  // ...
  var origScale = g_shipSprite.scale;
  // pass my scale into the sprite, for drawing
  g_shipSprite.scale = this._scale;
  g_shipSprite.drawWrappedCentredAt(
    ctx, this.cx, this.cy, this.rotation
  );
  g_shipSprite.scale = origScale;
};

Ship.prototype.toClick = function(){
...
function processDiagnostics() {
  ...
  return g_useExtras && g_useGravity && g_allowMixedActions;
}
